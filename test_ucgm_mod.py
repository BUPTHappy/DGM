#!/usr/bin/env python3
"""
Test script to verify the UCGM mod attribute fix.
This script tests that the UCGM mod attribute is properly initialized to match checkpoint structure.
"""
...
def test_ucgm_mod_attribute(checkpoint_path, device="cuda:0"):
    """
    Test that UCGM mod attribute is properly initialized to match checkpoint structure.
    """
    print(f"Testing UCGM mod attribute with: {checkpoint_path}")
    print(f"Using device: {device}")
    
    # Create temp output dir
    temp_output_dir = tempfile.mkdtemp(prefix="test_ucgm_mod_")
    ...
    try:
        ...
        success_indicators = [
            "Auto-detected UCGM mode from checkpoint",
            "Using UCGM: True",
            ...
            "Loading model",
            "Loading ema_model",
            "Using EMA policy for evaluation."
        ]
        ...
        failed_indicators = [
            ...
            "RuntimeError",
            "Cannot find autoregressive_model_params"
        ]
        ...
        if failed_count > 0:
            print("\n❌ PROBLEM FOUND: UCGM mod attribute issue!")
            print("The UCGM mod attribute is not properly initialized to match checkpoint structure.")
            return False
        elif success_count >= 6:  # Most success indicators present
            print("\n✅ SUCCESS: UCGM mod attribute is properly initialized!")
            print("UCGM model structure matches checkpoint and loads successfully.")
...
if __name__ == "__main__":
    ...
    success = test_ucgm_mod_attribute(checkpoint_path)
    
    if success:
        print(f"\n🎉 UCGM mod attribute fix is working!")
        print("UCGM mod attribute is properly initialized to match checkpoint structure.")
        print("Checkpoint loading should now work without errors.")
    else:
        print(f"\n❌ UCGM mod attribute fix needs more work.")
        print("UCGM mod attribute still doesn't match the checkpoint structure.")
