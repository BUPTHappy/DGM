...
class DiTPolicyBlock(nn.Module):
    ...
    def forward(self, x_act, x_cond, t_emb, attn_mask=None, key_pad_mask=None):
        # x_act: [B, L_act, D]; x_cond: [B, L_cond, D]; t_emb: [B, D]
        shift, scale = self.t2affine(t_emb).chunk(2, dim=-1)  # [B,D], [B,D]
        shift = shift[:, None, :]  # [B,1,D]   (broadcast over L_act)
        scale = scale[:, None, :]  # [B,1,D]

        q  = self.ln_q(x_act) * (1 + scale) + shift
        ...
        x = x + self.g_mlp  * self.mlp(self.ln_mlp(x) * (1 + scale) + shift)
        return x
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # AdaLN-Zero: zero-init the last layer in each block's t2affine
    ...
    def forward(
        ...
        # Blocks
        for blk in self.blocks:
            x = blk(x, cond, t_emb, attn_mask=attn_mask, key_pad_mask=key_pad_mask)
