...
from unified_video_action.model.ucgm.transports import TRANSPORTS
...
class UCGMTS(torch.nn.Module):
    ...
    def training_step(self, model, x=None, c=None):
        if x.ndim == 3:
            t = self.sample_beta(self.tdc[0], self.tdc[1], [x.size(0), 1, 1]).to(x) 
        else:
            t = self.sample_beta(self.tdc[0], self.tdc[1], [x.size(0), 1]).to(x)
        t = torch.clamp(t * self.tdc[2], min=0, max=1)
        z = torch.randn_like(x)
        nullc = torch.zeros_like(c[0])
        ...
        with torch.no_grad():
            ...
            if self.enr != 0.0:
                if self.ens == "fc-vs-fe":  # To learning enhanced target score function
                    ...
                    refer_x, refer_z, _, _ = self.forward(self.mod, x_t, t, **dict(c=e))
                    torch.cuda.set_rng_state(rng_state)
                    predc_x, predc_z, _, _ = self.forward(self.mod, x_t, t, **dict(c=c))
            ...
            if self.cor != 0.0:
                ...
                def xfunc(r):
                    ...
                    _, _, F_th_r, den_r = self.forward(model, xr, r, **dict(c=c))
                    if self.enr != 0.0:
    ...
    def sampling_loop(
        ...
        **model_kwargs,
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            # First order prediction
            x_hat, z_hat, _, _, model_var_values = self.forward(
                sampling_model,
                x_cur.to(torch.float32),
                t_cur.to(torch.float32),
                **model_kwargs,
            ...
            if stochast_ratio == "MODEL_GUIDED":
                frac = (model_var_values + 1) / 2
                model_log_variance = frac * self.max_log(t_cur) + (1 - frac) * self.min_log(t_cur)
                st_ratio = torch.exp(model_log_variance * 0.5)
                noi = torch.randn(x_cur.size()).to(x_cur)

            elif stochast_ratio == "SDE":
                stochast_ratio = (
                    torch.sqrt((t_next - t_cur).abs())
                    * torch.sqrt(2 * self.alpha_in(t_cur))
                    / self.alpha_in(t_next)
