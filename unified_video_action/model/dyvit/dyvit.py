...
def get_avg_fused_tokens(x):
    data = x[..., :-1]  # (B, T, C)
    index_channel = x[..., -1]  # (B, T)

    mask = torch.abs(torch.minimum(x[..., -1], torch.zeros_like(index_channel)))

    # Sum the masked entries along dim=1:
    #   mask.unsqueeze(-1) -> (B, T, 1), broadcasts over C
    masked_sum = (data * mask.unsqueeze(-1)).sum(dim=1)  # (B, C)

    # Count how many −1’s per batch:
    counts = mask.sum(dim=1, keepdim=True)  # (B, 1), int

    # Compute the per-batch mean over only the −1-marked rows:
    masked_mean = masked_sum / counts  # (B, C)
    return masked_mean
...
#     return compl
def add_embed_pruned(x: torch.Tensor, pos_embed: torch.Tensor):
    """
    x: (B, T, C+1)  — features in [:, :, :-1], integer indices in [:, :, -1]
    pos_embed: (1, P, C)  — positional embeddings
    """
    # 1. pull out the position indices
    positions = x[..., -1].long()  # (B, T)
    valid = positions >= 0  # (B, T)
    idx = positions.clamp(min=0)  # (B, T)

    # 2. gather the pos-embeddings and mask invalid slots
    pe = pos_embed[0][idx]  # (B, T, C)
    pe = pe * valid.unsqueeze(-1)  # zeros where pos==-1

    # 3. add out-of-place into the feature channels
    features = x[..., :-1]  # (B, T, C)
    new_features = features + pe  # (B, T, C)

    # 4. stitch back on the original index channel (unchanged)
    return torch.cat([new_features, x[..., -1:]], dim=-1)


def complement_idx(idx: torch.Tensor, dim: int) -> torch.Tensor:
    """
    For each “row” in idx (i.e. over the trailing dimension), return the sorted
    complement of idx in [0, 1, …, dim-1], but pad every row to the same length
    (the max complement‐length in the batch) by repeating that row's last value.

    Args:
        idx:  LongTensor of shape [..., K]
        dim:  int, the size of the full universe {0,1,...,dim-1}

    Returns:
        LongTensor of shape [..., M] where
          M = max_i ( dim - unique_count(idx_i) )
    """
    shape0 = idx.shape[:-1]
    K = idx.shape[-1]
    flat = idx.view(-1, K)
    ...
    # 1) build each row’s unique set and compute its complement
    comps = []
    for row in flat:
        u = torch.unique(row)  # unique idx in this row
        mask = torch.zeros(dim, dtype=torch.bool, device=device)
        mask[u] = True  # True for values to REMOVE
        comp = torch.arange(dim, device=device)[~mask]
        comps.append(comp)

    # 2) figure out the max complement length
    max_len = max(c.numel() for c in comps)

    # 3) pad each to max_len by repeating its last element
    padded = []
    for c in comps:
        if c.numel() < max_len:
            pad_cnt = max_len - c.numel()
            c = torch.cat([c, c[-1].repeat(pad_cnt)])
        padded.append(c)

    out = torch.stack(padded, dim=0)
    return out.view(*shape0, max_len)
